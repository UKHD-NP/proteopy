...
def pairwise_peptide_correlations_(
    ...
    """Calculate pairwise peptide correlations. Only outputs unique
    (non-symmetrical) correlations.
    ...
    for prot, gprot in per_batch_df.reset_index().groupby(
        ...
        for (pa, pb), gp in gprot.groupby(
            ...
            if (mask.sum() >= min_contrib_batches) and (wsum >= min_wsum):
                # Fixed-effects mean (zbar_fe) and weighted between-batch
                # variance (var_z_between)
                zbar_fe = float((w * z).sum() / wsum)
                Q = float((w * (z - zbar_fe) ** 2).sum())
                var_z_between = Q / wsum

                # Conservative PCC from fixed-effects mean (no DL): shift by
                # var_z_between
...
def peptide_dendograms_by_correlation(
    ...
    if "pairwise_peptide_correlations" not in adata.uns:
        raise ValueError("pairwise_peptide_correlations not in .uns")
    ...
    for protein_id, df in corrs.groupby("protein_id", observed=True):
        ...
        dends[protein_id] = peptide_dendograms_by_correlation_(
            corr_dists, method="agglomerative-hierarchical-clustering"
        )
...
def peptide_clusters_from_dendograms_(
    ...
    """Cut clusters from cluster_peptides into N clusters with more than
    1 peptide."""
...
def proteoform_scores_(
    ...
    """Calculates a score for proteoforms based on the difference of
