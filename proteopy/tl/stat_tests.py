...
def _perform_anova(
    ...
    equal_var : bool, optional
        If True, perform standard one-way ANOVA assuming equal
        population variances. If False, perform Welch's ANOVA.
...
def differential_abundance(
    ...
    method : str, optional
        ...
        - ``"anova_oneway"``: One-way ANOVA assuming equal
          variances, testing whether any group differs from the
          others. Requires at least 3 groups. Only supports
          ``setup`` with ``"groups"`` key (or ``None``/``{}``
          for all groups).
        - ``"anova_oneway_welch"``: Welch's one-way ANOVA, which
          does not assume equal population variances. Otherwise
          identical to ``"anova_oneway"``.
    group_by : str
