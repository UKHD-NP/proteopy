...
def reconstruct_corrs_df_symmetric_from_long_df(
    df, var_a_col=0, var_b_col=1, corr_col=2
):
    ...
    for i in range(n):
        for j in range(i + 1, n):
            ...
            elif np.isnan(corr_matrix[j, i]) and not np.isnan(
                corr_matrix[i, j]
            ):
            ...
            elif not np.isnan(corr_matrix[j, i]) and not np.isnan(
                corr_matrix[i, j]
            ):
                assert corr_matrix[i, j] == corr_matrix[j, i]

    corr_df = pd.DataFrame(
        corr_matrix, index=all_peptides, columns=all_peptides
    )
