...
def _make_adata_filter_obs_base() -> AnnData:
    """Six obs, five vars with increasing missingness; some zeros
    present."""
    n = np.nan
    X = np.array(
        [
...
def _make_adata_filter_obs_groupby_na() -> AnnData:
    """Same as `_make_adata_filter_obs_groupby` but with an added NA
    group of four vars."""
    n = np.nan
    X = np.array(
        [
...
def _make_adata_filter_var_base() -> AnnData:
    """Five obs, six vars with increasing missingness across vars; some
    zeros."""
    n = np.nan
    X = np.array(
        [
...
def _make_adata_filter_var_groupby() -> AnnData:
    """Five obs with obs groupings; vars differ in completeness per
    group."""
    n = np.nan
    X = np.array(
        [
...
def _make_adata_filter_var_groupby_na() -> AnnData:
    """Same as `_make_adata_filter_var_groupby` but with an added NA
    group of four obs."""
    n = np.nan
    X = np.array(
        [
...
def _make_adata_rzv_base() -> AnnData:
    """6 obs × 5 vars.  p0-p2 vary, p3 constant, p4 near-constant
    (<1e-8).
...
def _make_adata_remove_contaminants_peptide_level() -> AnnData:
    """4 obs × 5 vars, peptide-level data for contaminant filtering
    tests."""
    X = np.array(
        [
            [10, 20, 30, 40, 50],
