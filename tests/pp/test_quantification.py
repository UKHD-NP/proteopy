...
class TestSummarizeModifications:
    ...
    def test_basic_grouping_strips_modifications(self):
        """Peptides with modification annotations are grouped by their
        stripped (bare) sequence."""
    ...
    def test_unmodified_peptide_passes_through(self):
        """A peptide without modifications remains unchanged after
        grouping."""
        pids = ["SOLO"]
        X = np.array([[42.0]])
        adata = _make_peptide_adata(
    ...
    def test_aggregation_methods(self):
        """All four methods produce correct aggregated values."""
        adata = _make_peptide_adata()
        expected = {
            "sum": np.array(
                [[3.0, 7.0], [11.0, 15.0]],
        ...
        for method, exp in expected.items():
            ...
            np.testing.assert_allclose(
                result.X,
                exp,
                err_msg=(f"method='{method}' produced " f"wrong values"),
            )

    def test_aggregation_methods_single_group(self):
        """Methods applied to a single group of three peptidoforms."""
        ...
        for method, exp in expected.items():
            ...
            np.testing.assert_allclose(
                result.X,
                exp,
                err_msg=(f"method='{method}' on single group"),
            )
    ...
    def test_inplace_true_modifies_original(self):
        """Inplace=True modifies the original AnnData and returns
        None."""
        adata = _make_peptide_adata()
    ...
    def test_skip_na_true_ignores_nan(self):
        """skip_na=True aggregates over non-NaN values only."""
        n = np.nan
    ...
    def test_skip_na_false_propagates_nan(self):
        """skip_na=False produces NaN when any group member is NaN."""
        n = np.nan
        X = np.array([[1.0, n, 3.0, 4.0]])
        adata = _make_peptide_adata(X=X, obs_names=["s1"])
    ...
    def test_sort_false_preserves_first_appearance_order(
        ...
        """Sort=False preserves the order of first appearance."""
        pids = [
            "ZZZ",
            "ZZZ (Ox)",
            "AAA",
    ...
    def test_keep_var_cols_none_has_default_columns_only(
        ...
        """With keep_var_cols=None, output .var has only the mandatory
        columns."""
        adata = _make_peptide_adata(
            extra_var_cols={
                "gene": ["G1", "G1", "G2", "G2"],
    ...
    def test_keep_var_cols_carries_over_extra_columns(self):
        """Specifying keep_var_cols includes those columns in the output
        .var."""
        adata = _make_peptide_adata(
            extra_var_cols={
                "gene": ["G1", "G1", "G2", "G2"],
    ...
    def test_keep_var_cols_joins_differing_values(self):
        ...
        result = summarize_modifications(
            adata,
            keep_var_cols=["source"],
            inplace=False,
        )
        assert result.var.loc["PEPTIDEA", "source"] == "db1;db2"
        assert result.var.loc["PEPTIDEB", "source"] == "db1"
    ...
    def test_keep_var_cols_all_nan_produces_nan(self):
        """When all values in a kept column are NaN for a group, the
        result is NaN."""
        adata = _make_peptide_adata(
            extra_var_cols={
    ...
    def test_zero_to_na_converts_zeros_before_aggregation(
        ...
        """Zeros are replaced with NaN before aggregation when
        zero_to_na=True."""
    ...
    def test_zero_to_na_with_skip_na_false(self):
        """zero_to_na=True combined with skip_na=False propagates NaN
        from zeros."""
        X = np.array([[0.0, 2.0, 3.0, 4.0]])
        adata = _make_peptide_adata(X=X, obs_names=["s1"])
    ...
    def test_layer_uses_specified_layer(self):
        """When layer is specified, data comes from that layer instead
        of .X."""
        adata = _make_peptide_adata()
        layer_data = np.array(
    ...
    def test_verbose_prints_message(self, capsys):
        """Verbose=True prints a status message to stdout."""
        adata = _make_peptide_adata()
    ...
    def test_sparse_output_has_correct_values(self):
        """Sparse output matches the expected dense values."""
        adata = _make_peptide_adata()
        adata.X = sparse.csr_matrix(adata.X)
    ...
    def test_n_peptidoforms_counts_variants(self):
        """n_peptidoforms reflects total variants per stripped sequence
        group."""
        adata = _make_peptide_adata()
        result = summarize_modifications(
            adata,
        ...
        assert val_a == 2
        val_b = result.var.loc["PEPTIDEB", "n_peptidoforms"]
        assert val_b == 2

    def test_n_peptidoforms_single_variant(self):
        """An unmodified peptide alone counts as 1 peptidoform."""
        pids = ["SOLO"]
        X = np.array([[1.0]])
        adata = _make_peptide_adata(
            X=X,
        ...
        assert result.var.loc["SOLO", "n_peptidoforms"] == 1

    def test_n_peptidoforms_three_variants(self):
        """Three peptidoforms sharing the same stripped sequence."""
        adata = _make_single_group_adata()
        result = summarize_modifications(
            adata,
            inplace=False,
        )
        assert result.var.loc["SEQA", "n_peptidoforms"] == 3
    ...
    def test_n_modifications_basic(self):
        ...
        val_a = result.var.loc["PEPTIDEA", "n_modifications"]
        assert val_a == 1
    ...
    def test_n_modifications_unmodified_contributes_zero(
        ...
        """A group with only unmodified peptides has 0 modifications."""
        pids = ["BARE"]
        X = np.array([[5.0]])
        adata = _make_peptide_adata(
            X=X,
        ...
        assert result.var.loc["BARE", "n_modifications"] == 0

    def test_n_modifications_same_mod_same_pos_dedup(self):
        """Same modification at same position across peptidoforms counts
        only once."""
        pids = [
            "ABC (Ox)DEF",
            "ABC (Ox)DEF (Ph)",
        ...
        val = result.var.loc["ABCDEF", "n_modifications"]
        assert val == 2

    def test_n_modifications_same_text_different_pos(self):
        """Same modification text at different positions counts as
        ...
        assert result.var.loc["ABCD", "n_modifications"] == 2

    def test_n_modifications_multiple_mods_in_one_peptide(
        self,
    ):
        """A single peptide with multiple modifications contributes all
        of them."""
        pids = ["A (Ox)B (Ph)C"]
        X = np.array([[1.0]])
        adata = _make_peptide_adata(
        ...
        assert result.var.loc["ABC", "n_modifications"] == 2
    ...
    def test_custom_mod_regex(self):
        """A custom regex strips different annotation formats."""
        pids = [
            "PEP[Ox]TIDE",
            "PEP[Ph]TIDE",
            "PEPTIDE",
        ...
        val_pf = result.var.loc["PEPTIDE", "n_peptidoforms"]
        assert val_pf == 3
        val_nm = result.var.loc["PEPTIDE", "n_modifications"]
        assert val_nm == 2

    def test_custom_mod_regex_no_matches(self):
        """When the regex matches nothing, peptides pass through
        unchanged."""
        adata = _make_peptide_adata()
        result = summarize_modifications(
            adata,
    ...
    def test_error_invalid_method(self):
        """An unsupported method string raises ValueError."""
        adata = _make_peptide_adata()
        with pytest.raises(
            ValueError,
    ...
    def test_error_both_zero_to_na_and_fill_na(self):
        """Setting both zero_to_na and fill_na raises ValueError."""
        adata = _make_peptide_adata()
        with pytest.raises(
            ValueError,
    ...
    def test_error_conflicting_protein_ids(self):
        """Peptides that strip to the same sequence but map to different
        protein_ids raise ValueError."""
    ...
    def test_error_keep_var_cols_missing_column(self):
        """keep_var_cols with a column not in adata.var raises
        KeyError."""
        adata = _make_peptide_adata()
        with pytest.raises(
            KeyError,
            match="not found in adata.var",
        ):
            summarize_modifications(
                adata,
                keep_var_cols=["nonexistent"],
            )

    def test_error_keep_var_cols_multiple_missing(self):
        """All missing keep_var_cols entries are reported."""
        adata = _make_peptide_adata()
        with pytest.raises(
            KeyError,
            match="nonexistent",
    ...
    def test_error_keep_var_cols_reserved_peptide_id(self):
        """keep_var_cols containing 'peptide_id' raises ValueError."""
        adata = _make_peptide_adata()
        with pytest.raises(
            ValueError,
            match="reserved columns",
        ):
            summarize_modifications(
                adata,
                keep_var_cols=["peptide_id"],
            )

    def test_error_keep_var_cols_reserved_protein_id(self):
        """keep_var_cols containing 'protein_id' raises ValueError."""
        adata = _make_peptide_adata()
        with pytest.raises(
            ValueError,
            match="reserved columns",
    ...
    def test_error_invalid_mod_regex(self):
        """A malformed regex raises ValueError with a descriptive
        message."""
        adata = _make_peptide_adata()
        with pytest.raises(
            ValueError,
            match="Invalid mod_regex",
        ):
            summarize_modifications(
                adata,
                mod_regex=r"(unclosed",
            )

    def test_error_layer_with_infinite_values(self):
        """A layer containing infinite values is rejected upfront."""
        adata = _make_peptide_adata()
        layer_data = np.array(
            [[1.0, np.inf, 3.0, 4.0], [5.0, 6.0, 7.0, 8.0]],
        )
    ...
    def test_var_column_named_stripped_not_clobbered(self):
        """A user .var column named '_stripped' is preserved when
        included via keep_var_cols."""
        adata = _make_peptide_adata(
            extra_var_cols={
                "_stripped": ["a", "b", "c", "d"],
            },
    ...
    def test_output_peptide_id_matches_var_names(self):
        """Output .var['peptide_id'] matches .var_names exactly."""
        adata = _make_peptide_adata()
        result = summarize_modifications(
            adata,
            inplace=False,
    ...
    def test_output_protein_id_is_single_mapped(self):
        """Each output peptide maps to exactly one protein_id (no multi-
        mapping)."""
        adata = _make_peptide_adata()
        result = summarize_modifications(
            adata,
    ...
    def test_all_nan_matrix_sum(self):
        """An all-NaN matrix produces all-NaN output with sum."""
        n = np.nan
        X = np.array([[n, n, n, n]])
        adata = _make_peptide_adata(
            X=X,
    ...
    def test_all_nan_matrix_mean(self):
        """An all-NaN matrix produces all-NaN output with mean."""
        n = np.nan
        X = np.array([[n, n, n, n]])
        adata = _make_peptide_adata(
            X=X,
    ...
    def test_all_zero_matrix_with_zero_to_na(self):
        """An all-zero matrix with zero_to_na=True produces all-NaN
        output for sum."""
        X = np.zeros((1, 4))
        adata = _make_peptide_adata(
            X=X,
    ...
    def test_mixed_nan_across_observations(self):
        """NaN patterns can differ across observations."""
        n = np.nan
        X = np.array(
            [[1.0, n, 3.0, 4.0], [n, 6.0, 7.0, 8.0]],
        )
    ...
    def test_sparse_with_nan_values(self):
        """Sparse input with stored NaN values aggregates correctly and
        the output remains sparse."""
        n = np.nan
