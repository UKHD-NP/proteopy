...
def compare_clusters_dsVlist(ds, ref):
    """Check if peptide cluster annotations defined in a pd.DataSeries
    are the same as a refrence list of clusters annotations.
...
def traces_preproc_anns():
    ...
    anns_path = (
        DATA_DIR
        / "mouse_tissue/traces_pre-processed_trace-annotations_rcopf.tsv"
    )
...
def fraction_annotation():
    frac_ann_path = DATA_DIR / "mouse_tissue/fraction_annotation.tsv"
...
def traces_corrs_ref(traces_corrs):
    """Filter COPF mouse tissue correlations df for unique (non-
    symmetrical) correlation values."""
    corrs_ref = traces_corrs.set_index("protein_id")
    corrs_ref = corrs_ref[corrs_ref["PCC"] != 1]

    def sort_peps_ab(row):
...
def test_pairwise_peptide_correlations_vs_rcopf(
    traces_preproc_ext, traces_corrs_ref
):
    """Test pairwise_peptide_correlations() application for equality to
    rCOPF correlations df.
    ...
    def pep_corrs(x):
        return pairwise_peptide_correlations_(
            x,
    ...
    corrs = traces_preproc_ext.groupby("protein_id").apply(
        pep_corrs, include_groups=False
    )
    ...
    pep_cols = ["pepA", "pepB"]
    assert corrs[pep_cols].equals(
        traces_corrs_ref[pep_cols]
    )  # Both prev. sorted

    abs_tolerance = 1e-14  # loaded reference corrs precision 1e-15
    assert corrs["PCC"].values == approx(
        traces_corrs_ref["PCC"].values, abs=abs_tolerance
    )
...
def prot_dends():
    ...
    clusts_ref_path = (
        DATA_DIR / "mouse_tissue/traces_cluster-dendograms_rcopf.json"
    )

    with open(clusts_ref_path) as f:
        dends_R = json.load(f)
...
def test_peptide_dendograms_by_correlation_vs_rcopf(traces_corrs, prot_dends):
    ...
    for protein_id, df in traces_corrs.groupby("protein_id"):
        ...
        corr_df_sym = reconstruct_corrs_df_symmetric_from_long_df(
            df, var_a_col="pepA", var_b_col="pepB", corr_col="PCC"
        )
    ...
    for prot_id, dend in dends_ref.items():
        ...
        dend_corrected = remap_dendogram_leaf_order(
            dend, ref_labels=dends[prot_id]["labels"]
        )
    ...
    for prot_id in dends_ref.keys():
        ...
        check_dendogram_equality(
            dends[prot_id], dends_ref[prot_id], abs_tolerance=abs_tolerance
        )


def test_peptide_clusters_from_dendograms_():
    """Test protein-level peptide_clusters_from_dendograms_() on a
    ...
    # cluster numbers may be different order, which is accounted for in test
    # comparisons.
    ...
    clusters = peptide_clusters_from_dendograms_(
        dendogram, n_clusters=1, min_peptides_per_cluster=1
    )
    ...
    # Config 2
    clusters = peptide_clusters_from_dendograms_(
        dendogram, n_clusters=1, min_peptides_per_cluster=2
    )
    expected_clusters = [["pepA", "pepB", "pepC", "pepD", "pepE", "pepF"]]
    compare_clusters_dsVlist(clusters, expected_clusters)
    ...
    # Config 3
    clusters = peptide_clusters_from_dendograms_(
        dendogram, n_clusters=2, min_peptides_per_cluster=1
    )
    expected_clusters = [["pepA", "pepB", "pepC", "pepD"], ["pepE", "pepF"]]
    compare_clusters_dsVlist(clusters, expected_clusters)
    ...
    # Config 4
    clusters = peptide_clusters_from_dendograms_(
        dendogram, n_clusters=2, min_peptides_per_cluster=2
    )
    expected_clusters = [["pepA", "pepB", "pepC", "pepD"], ["pepE", "pepF"]]
    compare_clusters_dsVlist(clusters, expected_clusters)
    ...
    # Config 5
    clusters = peptide_clusters_from_dendograms_(
        dendogram, n_clusters=3, min_peptides_per_cluster=1
    )
    expected_clusters = [["pepA", "pepB"], ["pepC", "pepD"], ["pepE", "pepF"]]
    compare_clusters_dsVlist(clusters, expected_clusters)
    ...
    # Config 6
    clusters = peptide_clusters_from_dendograms_(
        dendogram, n_clusters=3, min_peptides_per_cluster=2
    )
    expected_clusters = [["pepA", "pepB"], ["pepC", "pepD"], ["pepE", "pepF"]]
    compare_clusters_dsVlist(clusters, expected_clusters)
...
@pytest.fixture
def prot_clust_ann():
    """Get protein-level peptide cluster annotations from COPF mouse
    tissue df."""
...
def test_peptide_clusters_from_dendograms_vs_rcopf_(
    prot_dends, prot_clust_ann
):
    """Test protein-level peptide_clusters_from_dendograms_() on an
    rCOPF-derived reference dataset."""
...
def trace_annotation_proteoform_scores():
    """Get protein-level proteoform annotations from COPF mouse tissue
    dataset."""
...
def test_proteoform_scores_vs_rcopf_(
    ...
    for prot, corrs in traces_corrs.groupby("protein_id"):
        ...
        corrs_mat = reconstruct_corrs_df_symmetric_from_long_df(
            corrs, var_a_col="pepA", var_b_col="pepB", corr_col="PCC"
        )
    ...
    proteoform_scores = proteoform_scores.loc[:, columns].set_index(
        "protein_id"
    )
    ...
    assert (
        len(
            set(proteoform_scores.columns).symmetric_difference(
                set(proteoform_scores_ref.columns)
            )
        )
        == 0
    )

    proteoform_scores = proteoform_scores.loc[
        proteoform_scores_ref.index, proteoform_scores_ref.columns
    ]
