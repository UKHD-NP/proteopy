...
def test_reconstruct_corrs_df_symmetric_from_long_df():
    ...
    df_expected = pd.DataFrame(
        {"a": [1, 0.1, 0.5], "b": [0.1, 1, 0.4], "c": [0.5, 0.4, 1]},
        index=["a", "b", "c"],
    )

    df_reconstructed = reconstruct_corrs_df_symmetric_from_long_df(
        df, "colA", "colB", 2
    )
...
def transform_dendogram_r2py(dendogram: dict):
    ...
    if "merge" not in dendogram.keys():
...
def remap_dendogram_leaf_order(dendogram: dict, ref_labels: list):
    ...
    assert set(orig_labels) == set(
        ref_labels
    ), f"orig_labels: {orig_labels},\nref_labels: {ref_labels}"
...
def check_dendogram_equality(
    dend, dend_ref, rel_tolerance=None, abs_tolerance=None
):
    """
    Note:
    ...
    # Correct dict keys
    assert all(
        [key in dend_ref.keys() for key in keys]
    ), f"dend.keys: {list(dend.keys())}\nkeys:{keys}"
    assert all(
        [key in dend.keys() for key in keys]
    ), f"dend.keys: {list(dend.keys())}\nkeys:{keys}"
